...
from gluon_kernels.rmsnorm.rms_norm_manual import rms_norm as gluon_rms_norm
from triton_kernels.rmsnorm.rmsnorm_cody import rms_norm as triton_rms_norm
...
def test_gluon_vs_triton(shape, dtype):
    """Verify that the Gluon kernel matches the Triton kernel output."""
    ...
    triton_out = triton_rms_norm(x, eps=1e-6, gamma=gamma)

    atol = 1e-2 if dtype == torch.float16 else 1e-5
    rtol = 1e-2 if dtype == torch.float16 else 1e-5

    assert torch.allclose(gluon_out, triton_out, atol=atol, rtol=rtol), (
        f"Gluon vs Triton mismatch for shape={shape}, dtype={dtype}\n"
        f"Max abs diff: {(gluon_out - triton_out).abs().max().item()}"
    )
