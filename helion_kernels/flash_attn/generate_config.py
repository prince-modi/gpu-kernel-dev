...
import torch
import helion
from flash_attn_helion import flashatt_fwd
...
attn_configs = [ 
        (
            #small config
            torch.randn((4,32,1024,64), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,1024,64), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,1024,64), dtype= torch.float16, device=DEVICE),
        ),
        (
            #medium config
            torch.randn((4,32,4096,64), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,4096,64), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,4096,64), dtype= torch.float16, device=DEVICE),
        ),
        (
            #large config
            torch.randn((4,32,16384,128), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,16384,128), dtype= torch.float16, device=DEVICE),
            torch.randn((4,32,16384,128), dtype= torch.float16, device=DEVICE),        
        )
]
...
if __name__ == "__main__":
    ...
    rmsnorm_functions = [flashatt_fwd]
    for func in rmsnorm_functions:
        autotune(func, attn_configs,force_autotune=True)